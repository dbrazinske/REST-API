...
const HouseholdApplianceModel = require('./household-appliance-model');
...
const cartItemValidationSchema = yup.object({
  householdApplianceId: yup.string().typeError('User.cartItems element.householdApplianceId must be a string')
    .required('User.cartItems element.householdApplianceId is required')
    .test(
      'is-mongo-object-id',
      'User.cartItems element.householdApplianceId must be valid MongoDB object Id',
      Types.ObjectId.isValid
    )
    .test(
      'householdAppliance-exists',
      'householdAppliance was not found using cartItems element.householdApplianceId ',
      async (householdApplianceId) => {
        const householdApplianceExists = await HouseholdApplianceModel.exists({ _id: householdApplianceId });

        return householdApplianceExists;
      }
    ),
...
const userValidationSchema = yup.object({
  ...
  favoredHouseholdAppliances: yup
    .array(yup.string().typeError('User.favoredHouseholdAppliances element must be a string')
      .required('User.favoredHouseholdAppliances element is required')
      .test(
        'is-mongo-object-id',
        'User.favoredHouseholdAppliances element must be valid MongoDB object Id',
        Types.ObjectId.isValid
      )),
